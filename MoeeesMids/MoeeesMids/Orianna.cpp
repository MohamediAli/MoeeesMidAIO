...
void Orianna::OnGameUpdate()
{
    ...
    auto target = GTargetSelector->FindTarget(QuickestKill, SpellDamage, Q->Range()+400);
    if(Extensions::Validate(target) && target->IsValidTarget())
    {
        eTarget = target;
    }
}
...
int Orianna::GetEHits()
{
    ...
    for(auto enemy : GEntityList->GetAllHeros(false, true))
    {
        ...
        {
            if(Extensions::SegmenDistance(enemy->ServerPosition().To2D(), NewOriannaBall.To2D(), Hero->GetPosition().To2D(), false, true) <= 110 * 110)
            {
...
bool Orianna::RLogic()
{
    ...
    if(KillStealR->Enabled() && DPS(target, true, true, true, true) >= target->GetHealth() && DPS(target, true, true, false, false,1) < target->GetHealth())
    {
        for(auto entry : hits)
        {
            for(auto champs : entry.second)
