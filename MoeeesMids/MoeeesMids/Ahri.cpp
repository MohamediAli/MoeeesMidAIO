...
Vec3 Ahri::RunPrediction (IUnit* Target, ISpell2* Skillshot)
{
    float TravelTime = (Target->GetPosition() - GEntityList->Player()->GetPosition()).Length2D() / Skillshot->Speed();
    Vec3 EstimatedMaxPosition;
    GPrediction->GetFutureUnitPosition (Target, TravelTime, true, EstimatedMaxPosition);
    auto BestCastPosition = EstimatedMaxPosition.Extend (Target->GetPosition(), Target->BoundingRadius() + Skillshot->Radius() * 0.9);
    return BestCastPosition;
}
...
void Ahri::CastE (IUnit* target)
{
    ...
    if (prediction_output.HitChance >= kHitChanceVeryHigh)
        {
        E->CastOnPosition (RunPrediction (target, E));
        }
}
