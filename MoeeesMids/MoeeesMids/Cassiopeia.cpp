...
Cassiopeia::Cassiopeia(IMenu* Parent, IUnit* Hero) :Champion(Parent, Hero)
{
    ...
    Q->SetSkillshot(0.3f, 160.f, FLT_MAX, 850.f);
...
float Cassiopeia::DPS(IUnit* target, bool dpsQ, bool dpsW, bool dpsE, bool dpsR, int eTimes, bool poison)
{
    ...
    auto GLP800 = GPluginSDK->CreateItemForId(3030, 800);
    auto Gunblade = GPluginSDK->CreateItemForId(3146, 700);
    if(Gunblade->IsOwned() && Gunblade->IsReady())
    {
        auto flDamage = static_cast<float>((175 + 4 * Hero->GetLevel())) + 0.3 * Hero->BonusMagicDamage();
        total += GDamage->CalcMagicDamage(Hero, target, flDamage);
    }
    if(GLP800->IsOwned() && GLP800->IsReady())
    {
        auto flDamage = static_cast<float>((100 + 6 * Hero->GetLevel())) + 0.35 * Hero->BonusMagicDamage();
        total += GDamage->CalcMagicDamage(Hero, target, flDamage);
    }
...
void Cassiopeia::qLogic(IUnit* target)
{
    if(target != nullptr)
    {
        ...
        Q->RunPrediction(target, false, kCollidesWithNothing, &prediction_output);
